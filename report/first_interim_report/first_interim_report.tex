...
NOTES:
\begin{enumerate}
    \item AND etc can have up to 16 inputs, XOR only 2 inputs
    \item Optional Monitor block (user can define it later)
    \item Talk about short vs long form "Is connected to", example B is short A is long
    \item clock cycle length is number of simulation cycles for clock to change
    \item "BAR" now only and always applies to dtype (change above paragraph), mention QBAR is opposite of Q
