...
\begin{itemize}
    \item AND, OR, NAND and NOR gates can have up to 16 inputs but XOR only has 2 inputs. 
    \item The monitor block is optional as the user can define it later as the user does not have to have an output. 
    \item When specifying connections, both "is connected to" and "to" can be used, e.g. "A is connected to B" or "A to B".  
    \item The clock cycle length $n$ is number of simulation cycles for clock to change. 
    \item The DTYPE has two outputs, Q and QBAR, where QBAR is the inverse of Q. 
\end{itemize}
