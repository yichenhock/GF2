...
\begin{lstlisting}
    ...
    device_block = "devices", bracket_open, {gate_definition | dtype_definition | switch_definition | clock_definition}, bracket_close, semicolon;
    ...
    connection_definition = (device_name | dtype_output_name | switch_name | clock_name), ("is connected to" | "to" | arrow), {(gate_input_name | dtype_input_name)}, semicolon;
...
\begin{table*}[ht]
\centering
\caption{Semantic errors, .\label{table:rotor}}
\vspace{\baselineskip}
...
\hline 
Each input channel can admit only a connection to one output & inputs: a to b.IN1, c to b.IN1 & Input channel b.IN1 can only connect to one output.\\
\hline 
...
\hline
Switch initial value or clock cycle length is not defined in initialisation & devices: sw1, sw1 are SWITCHES & Please define switches sw1, sw2 initial values.\\
\hline
...
\hline
Switches initialised in a way that breaks gate logic & devices: a is NOT; connections: sw1 to a, a to sw2; initialise: sw1 is HIGH, sw2 is HIGH & Please check switch initialisation for sw1. Initialisation does not conform to gate logic.\\
\hline
\end{tabular}
\end{table*}

Avoided words list: "to", "connected", "is", "are", "have", "has", "inputs", "input", "connections", "initialise", "device", "monitors", "cycle", "length".

Note that for all devices other than dtype, by definition, the number of outputs is 1. One output can be included in more than one connection, so it is not a semantic error if the same switch is connected as an input to more than one input port.
...
\begin{enumerate}
    \item A self-connected 'NOT' gate acts as a clock with an infinitely fast cycle frequency, as no delay is specified for all gates.
    \item If both SET and CLEAR of dtype are set to 1 at the same time have runtime error
\end{enumerate}
