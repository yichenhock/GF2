...
class DummyParser:
    ...
    def parse_network(self):
        ...
        self.devices.make_device(
            self.sw1.id, self.devices.SWITCH, self.init_state1)
        self.devices.make_device(
            self.sw2.id, self.devices.SWITCH, self.init_state2)
        ...
        self.network.make_connection(
            self.a.id, self.names.query('I1'), self.sw1.id, None)
        self.network.make_connection(
            self.a.id, self.names.query('I2'), self.sw2.id, None)
