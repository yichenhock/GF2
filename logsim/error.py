"""Contains inherited classes from Python's builtin Exceptions class, allowing raises to be thrown in parse.py."""
...
class UndefinedError(ParserSemanticError):
    """Devices not defined in devices block are mentioned in subsequent sections."""
...
class WrongDeviceName(ParserSemanticError):
    """Device has been defined as a gate and is lowercase but has the name of a switch or clock."""
    def __init__(self, symbol, name):
...
class InvalidClockLength(ParserSemanticError):
    """Clock cycle half length (the variable named 'clock length' for short) not in allowed range."""
...
class WrongSwitchName(ParserSemanticError):
    """Device has been defined as a switch but does not follow switch naming syntax."""
    def __init__(self, symbol, name):
...
class WrongClockName(ParserSemanticError):
    """Device has been defined as a clock but does not follow clock naming syntax."""
    def __init__(self, symbol, name):
...
class InvalidInputNumber(ParserSemanticError):
    """Device is a gate which is not XOR or NOT, and has got less than 1 or more than 16 inputs as defined in the initialise block."""
    def __init__(self, symbol):
...
class AttemptToDefineXORInputs(ParserSemanticError):
    """"User attempts to specify number of XOR inputs. This is always 2 and does not need to be defined."""
    def __init__(self, symbol):
...
class AttemptToDefineNOTInputs(ParserSemanticError):
    """User attempts to specify number of NOT inputs. This is always 1 and does not need to be defined."""
    def __init__(self, symbol):
...
class AttemptToDefineDTYPEInputs(ParserSemanticError):
    """User attempts to specify number of DTYPE inputs. This is always 4 and does not need to be defined."""
    def __init__(self, symbol):
...
class DeviceNotInitialised(ParserSemanticError):
    """A gate or DTYPE which has been defined in the devices section is skipped in the initialise section."""
    def __init__(self, symbol, name):
...
class SwitchNotInitialised(ParserSemanticError):
    """A switch which has been defined in the devices section is skipped in the initialise section."""
    def __init__(self, symbol, name):
...
class ClockNotInitialised(ParserSemanticError):
    """"A clock which has been defined in the devices section is skipped in the initialise section."""
    def __init__(self, symbol, name):
...
class InvalidInitClockRule(ParserSyntaxError):
    """Clock initialisation line is missing the keywords required: 'cycle'
...
class DeviceTypeError(ParserSyntaxError):
    """The symbol in place of expected device type keyword in a line in the devices block is not in the list below.
...
class InvalidDeviceName(ParserSyntaxError):
    """The symbol in place of an expected device name in a line in the devices block is not a name type (lowercase, mixed letters and numbers, first character is a letter)."""
    def __init__(self, symbol):
...
class InvalidClockName(ParserSyntaxError):
    """"Device defined as a clock does not have a name of the form 'clk' followed by a number."""
    def __init__(self, symbol):
...
class InputPortError(ParserSyntaxError):
    """Input port is name is not valid. This could be because it is not of the form 'I' followed by a number for non-DTYPES.
    
    For DTYPEs, this is because it is not one of 'CLK', 'SET', 'CLEAR' and 'DATA'. 
    ...
    For other gates, this could be because the number exceeds the defined number of inputs as in the initialise section.
    """
    def __init__(self, symbol):
...
class DotError(ParserSyntaxError):
    ...
    def __init__(self, symbol):
        message = "Missing dot in input name. Input names should be of the form device_name.port_name."
        super().__init__(symbol, message)
