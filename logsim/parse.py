...
class Parser:
    ...
    def connections_sub_block(self):
        ...
        Starts at point where subheader name has been read
        """
        ...
        if self.symbol.type == self.scanner.DOT and self.object_dict[self.current_name] == "DTYPE":
            ...
            if self.symbol.id in self.dtype_outputs:
                ...
                # Cannot have multiple of same signal name for d-type
                    ...
                    self.semantic.printerror(self.semantic.SIGNAL_ALREADY_EXISTS)
                    return False

                print(self.signal_names)
                if block == "monitors" and signal_name not in self.signal_names:
                ...
                self.syntax.printerror(self.syntax.DTYPE_OUTPUT_NAME_ERROR)
                return False
        
        elif self.symbol.type == self.scanner.DOT and self.object_dict[self.current_name] != "DTYPE":
            self.syntax.printerror(self.syntax.DOT_UNEXPECTED)
            return False
    ...
    def monitors_block(self):
        ...
        if self.symbol.type == self.scanner.CLOSE_BRACKET:
            ...
            if self.symbol.type == self.scanner.EOF:
                ...
                self.syntax.printerror(self.syntax.EXTRA_INFORMATION_AFTER_MONITORS)
                return True
        else:
            self.syntax.printerror(self.syntax.NO_CLOSE_BRACKET)
...
#===========================================================================================================
    def connection_definition(self, subsection, device_type):
        ...
        if self.symbol.id == self.scanner.to_id:
            ...
            self.symbol = self.scanner.get_symbol()
            print("Symbol after 'to':", self.names.get_name_string(self.symbol.id))

            if device_type != "DTYPE":
                print("Connecting to non dtype gate")
                self.output_device_port_id = None

                print("Calling gate_input_name")
            ...
            # If connecting to dtype:                   
                ...
                print("Connecting to dtype gate")
                self.output_device_port_id = None
            ...
            if self.symbol.id == self.scanner.connected_id:
                ...
                if self.symbol.id == self.scanner.to_id:
                    ...
                    self.symbol = self.scanner.get_symbol()
                    print("Device name:", self.names.get_name_string(self.symbol.id))
    ...
    def gate_input_name(self, subsection):
        ...
        Starts when already obtained the name of the device to have an input port. This function checks if it matches the section subheader.
            ...
            if self.symbol.type == self.scanner.NAME:
                ...
                if port_name[0] == "I":
                    for char in port_name[1: len(port_name)]:
                        if not char.isdigit():
                            self.syntax.printerror(self.syntax.PORT_NAME_ERROR)
                            return
                    ...
                    self.syntax.printerror(self.syntax.NO_INPUT_PORT_NAME)
                    return
            else:
                self.syntax.printerror(self.syntax.NO_INPUT_PORT_NAME)
                return
        else:
            self.syntax.printerror(self.syntax.MISSING_DOT_INPUT)
            return
    ...
    def device_initialisation(self, currentname):
        ...
        if self.symbol.type == self.scanner.KEYWORD and self.symbol.id in self.possession_ids:
            ...
            if self.symbol.type != self.scanner.NUMBER:
                self.syntax.printerror(self.syntax.INPUT_NUMBER_ERROR)
                return False
        ...
        # If next symbol is not a possession keyword, throw error        
            ...
            self.syntax.printerror(self.syntax.NO_POSSESSION_KEYWORD)
            return False
    ...
    def switch_initialisation(self, currentname):
        ...
        if self.symbol.type == self.scanner.KEYWORD and self.symbol.id in self.definition_ids:
            ...
            if self.symbol.id not in self.switch_level:
                self.syntax.printerror(self.syntax.SWITCH_LEVEL_ERROR)
                return False
        ...
        if self.symbol.type == self.scanner.KEYWORD and self.symbol.id == self.other_keywords_ids[4]:
            ...
            if self.symbol.type == self.scanner.KEYWORD and self.symbol.id == self.other_keywords_ids[5]:
                ...
                if self.symbol.type == self.scanner.NUMBER:
                    ...
                    self.syntax.printerror(self.syntax.NO_CYCLE_LENGTH)
                    return False
            else:
                self.syntax.printerror(self.syntax.NO_LENGTH_KEYWORD)
                return False
        # If next symbol is not a possession keyword, throw error        
        else:
            self.syntax.printerror(self.syntax.NO_CYCLE_KEYWORD)
            return False
...
#===========================================================================================================
    ...
    def read_name(self, block):
        ...
        if self.symbol.type == self.scanner.NAME:
            ...
            if block == "connections subheading":
                ...
                if self.is_legal_name and self.current_name not in self.object_dict:
                    ...
                    self.is_legal_name = False
                    self.syntax.printerror(self.syntax.CONNECTION_SUBHEADER_NAME_ERROR)
                    return
                elif self.is_legal_name and self.current_name in self.object_dict:
                    return 
        ...
        # No device name found at the beginning of the line inside the device class but not reached end of block yet
            ...
            self.syntax.printerror(self.syntax.NO_DEVICE_NAME)
            self.is_legal_name = False
            return
